...
const Button = defineComponent({
  ...
  setup(props, { slots }) {
    ...
    return () => {
      if (useLink.value) {
        return (
          <a class={btnClass.value} href={props.disabled ? undefined : props.href} target={props.target} rel={props.rel} onClick={onClick}>
      ...
      return (
        <button type={props.htmlType} class={btnClass.value} disabled={props.disabled || props.loading} onClick={onClick}>
          {/* {props.icon ? <Icon class="sk-btn-icon" type={props.icon} /> : null} */}
          {slots.default?.()}
        </button>
      );
